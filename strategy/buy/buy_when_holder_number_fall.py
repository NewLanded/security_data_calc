        ...
        if holder_number_data == sorted(holder_number_data, reverse=True):
            if sum([security_point_data["close"].iloc[-1], security_point_data["close"].iloc[-2], security_point_data["close"].iloc[-3]]) > sum(
                    [security_point_data["close"].iloc[-4],
                     ...
                     security_point_data["close"].iloc[-6]]):
                if security_point_data["close"].iloc[-1] > holder_number_date_point['close'].iloc[-1]:
...
def start(date_now=None):
    ...
    if Date().is_workday(date_now):
        ts_codes = Security().get_efficient_ts_code_by_column_name_list(["normal_status", "tactics_5_status", "tactics_6_status"])
...
if __name__ == "__main__":
    start_date, end_date = datetime.datetime(2019, 9, 6), datetime.datetime(2016, 5, 8)
    start(start_date)
