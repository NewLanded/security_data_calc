...
def start(all_ts_code, date):
    for ts_code in all_ts_code:
        ...
        for window_num in range(3, 12):
            target_data_now = qfq_security_point_data["close"].rolling(window=window_num).apply(lambda x: x[0] > x[-1], raw=True).values
            target_data_list.append(target_data_now)
        ...
        }, index=qfq_security_point_data["trade_date"]).values
